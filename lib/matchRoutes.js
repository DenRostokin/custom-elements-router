...
var matchRoutes = function matchRoutes(routes, pathname) {
  return routes.reduce(function (acc, route) {
    var match = route.path ? (0, _matchPath.default)(pathname, route) : acc.length ? acc[acc.length - 1].match : _index.default.computeRootMatch(pathname);
    ...
    if (match) {
      ...
      return [{
        route: route,
        match: match
      }].concat(_toConsumableArray(routesMatch), _toConsumableArray(acc));
    }
